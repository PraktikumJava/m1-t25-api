package Sprint8.Them2_API.Lesson6.Task3;

/**
 * Вы практически завершили реализацию HTTP-сервера. Осталось написать обработку эндпоинта для добавления комментариев.
 * Для этого реализуйте метод handlePostComments.
 * В процессе реализации учтите обработку следующих ситуаций:
 * Если комментарий успешно добавлен, то ответ должен содержать текст Комментарий добавлен, а его код должен быть равен 201.
...
/** Подсказка
 * Чтобы получить тело запроса, используйте метод getRequestBody класса HttpExchange. Он возвращает объект InputStream,
 * у которого есть метод readAllBytes. Используйте его для преобразования тела запроса в строку.
            ...
            try {
                ...
                for (Post post : posts) {
                    if (post.getId() == postId) {
                        post.getCommentaries().add(comment);
                        postFound = true;
                        break;
                    }
                }
